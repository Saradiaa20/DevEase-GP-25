...
class TechnicalDebtMetrics:
    ...
    # FIX Issue 3: renamed from 'debt_trend' to 'debt_severity' to accurately reflect
    # what this field represents. Without historical data, we cannot compute a real
    # trend (increasing/decreasing over time). This field categorizes the CURRENT
    # debt level only. Renamed in the dataclass, calculator, and all usages below.
    debt_severity: str  # 'critical', 'high', 'moderate', 'low' — current severity level
...
class TechnicalDebtCalculator:
    ...
    def calculate_debt(
        ...
        # FIX Issue 3: use _get_debt_severity() instead of _determine_debt_trend().
        # The old method returned 'increasing'/'stable'/'decreasing' which implied
        # historical comparison — but it was just re-labeling the current score.
        # Now it returns honest severity labels: 'critical', 'high', 'moderate', 'low'.
        debt_severity = self._get_debt_level(total_debt_score)
        ...
        return TechnicalDebtMetrics(
            ...
            debt_severity=debt_severity,   # FIX Issue 3: was debt_trend
            recommendations=recommendations
        )

    def _calculate_smell_debt(self, code_smells: Optional[Dict[str, Any]]) -> float:
        """Calculate debt from code smells"""
        # FIX Bug 1: original guard was `code_smells.get('total_smells', 0) == 0`
        # but smell_summary in feature_router never included 'total_smells', so this
        # always returned 0.0. Now we check 'by_severity' which is always populated.
        if not code_smells or not code_smells.get('by_severity'):
            return 0.0
    ...
    def _calculate_documentation_debt(
        ...
        if ast_data:
            ...
            if total_elements > 0:
                estimated_missing_docs = max(0, total_elements * 0.3)
    ...
    def _estimate_fix_hours(
        ...
        """Estimate hours needed to fix all issues"""
        total_hours = 0.0

        if code_smells:
            # FIX Bug 2: original code used code_smells.get('smells', []) but
            # smell_summary in feature_router never had a 'smells' key, so this
            # always iterated over an empty list, making estimated_hours only ever
            # count quality issues. Now 'smells' is populated in feature_router.
    ...
    def _get_priority_issues(
        ...
        if code_smells:
            # FIX Bug 2: same fix as _estimate_fix_hours — 'smells' key was never
            # present in the old smell_summary, so priority_issues was always empty.
            smells_list = code_smells.get('smells', [])
    ...
    def _generate_recommendations(
        ...
        if quality_score:
            ...
            recommendations.extend(quality_recommendations[:3])
    ...
    def get_debt_summary(self, metrics: TechnicalDebtMetrics) -> Dict[str, Any]:
        ...
        return {
            ...
            'debt_severity': metrics.debt_severity,   # FIX Issue 3: was debt_trend
            'recommendations_count': len(metrics.recommendations),
            'priority_issues': metrics.priority_issues[:5],
